...
class AudioHandlerService extends BaseAudioHandler with SeekHandler {
  ...
  Future<void> playNote(String assetPath) async {
    ...
    final item = MediaItem(
      ...
      artUri: Uri.parse('asset:///assets/images/icon_app.png'),
      playable: true,
  ...
  void _updatePlaybackState() {
    ...
    // Calcular el estado de procesamiento dominante
    AudioProcessingState combinedState = AudioProcessingState.idle;
    if (_player1.processingState == ProcessingState.ready || _player2.processingState == ProcessingState.ready) {
      combinedState = AudioProcessingState.ready;
    } else if (_player1.processingState == ProcessingState.buffering || _player2.processingState == ProcessingState.buffering) {
      combinedState = AudioProcessingState.buffering;
    } else if (_player1.processingState == ProcessingState.loading || _player2.processingState == ProcessingState.loading) {
